...
open(const char *path, int mode)
{
	...
	if ((r = sys_page_alloc(0, fd, PTE_P|PTE_U|PTE_W)) < 0)
		return r;
	fsipcbuf.open.req_omode = mode;
	memset(fsipcbuf.open.req_path,0,sizeof(fsipcbuf.open.req_path));
	strncpy(fsipcbuf.open.req_path,path,len);
...
devfile_read(struct Fd *fd, void *buf, size_t n)
{
	...
	if((r = fsipc(FSREQ_READ, NULL)) < 0)
		return r;
	memmove(buf,fsipcbuf.readRet.ret_buf,n);
